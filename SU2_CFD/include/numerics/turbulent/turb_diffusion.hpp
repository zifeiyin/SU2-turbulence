...
/*!
 * \class CAvgGrad_TurbKW
 * \brief Class for computing viscous term using average of gradient with correction (Wilcox k-omega turbulence model).
 * \ingroup ViscDiscr
 * \author A. Bueno.
 */
template <class FlowIndices>
class CAvgGrad_TurbKW final : public CAvgGrad_Scalar<FlowIndices> {
private:
  using Base = CAvgGrad_Scalar<FlowIndices>;
  using Base::Laminar_Viscosity_i;
  using Base::Laminar_Viscosity_j;
...
public:
   ...
   * \param[in] config - Definition of the particular problem.
   */
  CAvgGrad_TurbKW(unsigned short val_nDim, unsigned short val_nVar,
